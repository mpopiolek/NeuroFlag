...
- **Detail**: Tabela schematu i instrukcja edycji dokumentują stare pole `recommendation_threshold` (single int). Administrator podmieniający normy wg tej instrukcji stworzy plik w legacy-formacie — który zostanie zaakceptowany przez migration fallback, ale cicho zignoruje jego wartość i użyje domyślnych progów (5/3/4/3). Nowy schemat używa `recommendation_rules` (obiekt z 4 progami). Pola `category_descriptions` i `observation_checklist` nie są udokumentowane w tabeli schematu.
